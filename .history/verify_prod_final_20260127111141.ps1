...
while (-not $healthy -and $retryCount -lt $maxRetries) {
    try {
        ...
        Write-Host "Health check failed, retrying in 5s... ($_)"
...
try {
    $response = Invoke-RestMethod -Uri "$baseUrl/api/affiliate/apply" -Method Post -Body $body -ContentType "application/json"
