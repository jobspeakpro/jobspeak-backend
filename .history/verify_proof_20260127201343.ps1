...
try {
    Write-Host "Checking Env Vars..."
    $envCheck = Invoke-RestMethod -Uri "$apiUrl/__admin/env-vars" -Method Get -Headers @{ "x-verify-key" = "temp-verify-123" }
    Write-Host "Available Keys: $($envCheck.keys -join ', ')"
}
catch {
    Write-Warning "Env Var Check Failed: $_"
}
