...
$supaKeyMatch = [regex]::Match($js, '["''](eyJ[a-zA-Z0-9-_]+\.[a-zA-Z0-9-_]+\.[a-zA-Z0-9-_]+)["'']')

if (-not $supaUrlMatch.Success -or -not $supaKeyMatch.Success) {
    Write-Error "Could not extract Supabase credentials from bundle"
    exit 1
}
