...
// The block to replace, using LF (backticks)
...
if (normalizedContent.indexOf(targetBlock) !== -1) {
    let newContent = normalizedContent.replace(targetBlock, replacementBlock);

    // Convert back to CRLF for consistency? Or just write LF. JS handles writing.
    // Be careful not to corrupt mixed endings elsewhere if unnecessary, but LF is generally safe.
    fs.writeFileSync(filePath, newContent, 'utf8');
    console.log("Successfully replaced block using normalized content.");
} else {
    console.error("Target block not found even after normalization.");
    // Debug
    const marker = '// Generate per-question breakdown';
    const idx = normalizedContent.indexOf(marker);
    if (idx !== -1) {
        console.log("Found marker at index", idx);
        console.log("Target block start expected:");
        console.log(targetBlock.substring(0, 100));
        console.log("Actual file content at marker:");
        console.log(normalizedContent.substring(idx, idx + 100));
    }
    process.exit(1);
}
