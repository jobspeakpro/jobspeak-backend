...
try {
    ...
    Write-Host "Seed Response Header (Commit): $commitHeader"
    if (-not $commitHeader) { Write-Error "MISSING COMMIT HEADER ON SEED RESPONSE" }

    # Check Body
    $seedJson = $seedResponse.Content | ConvertFrom-Json
    Write-Host "Seed Body OK: $($seedJson.ok)"
