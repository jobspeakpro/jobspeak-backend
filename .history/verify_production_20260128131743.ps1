...
# Find bundle.js
$match = [regex]::Match($html, 'src="(/assets/index-[^"]+\.js)"')
if (-not $match.Success) { Write-Error "Could not find bundle.js in HTML"; exit 1 }
$bundlePath = $match.Groups[1].Value
$bundleUrl = "https://jobspeakpro.com$bundlePath"

Write-Host "Fetching bundle: $bundleUrl"
$js = (Invoke-WebRequest -Uri $bundleUrl -UseBasicParsing).Content

# Extract Supabase URL and Key
$supaUrlMatch = [regex]::Match($js, '(https:\/\/[a-z0-9-]+\.supabase\.co)')
# Regex matching eyJ until a quote (single or double)
$supaKeyMatch = [regex]::Match($js, '["''](eyJ[^"''`]+)["'']')

Write-Host "JS Length: $($js.Length)"

if (-not $supaUrlMatch.Success) { Write-Error "No URL match"; exit 1 }
if (-not $supaKeyMatch.Success) { Write-Error "No Key match"; exit 1 }

$supaUrl = $supaUrlMatch.Groups[1].Value
$supaKey = $supaKeyMatch.Groups[1].Value
Write-Host "Supabase URL found: $supaUrl"
Write-Host "Supabase Key found: $(($supaKey).Substring(0,10))..."
