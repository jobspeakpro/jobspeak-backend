...
async function setHeardAboutToNull(email) {
    ...
    // Explicitly select returned data to count rows
    const { data, error } = await supabaseAdmin
        .from('profiles')
        .update({ heard_about_us: null })
        .eq('id', user.id)
        .select(); // Ask for return data

    if (error) console.log(`   ❌ Reset Error for ${email}: ${error.message}`);
    else if (data.length === 0) console.log(`   ⚠ WARNING: 0 rows updated for ${email} (ID: ${user.id})! Profile might not exist.`);
...
async function verifyNull(email, quiet) {
    ...
    if (error) {
        ...
        if (!quiet) console.log(`   🔍 DB Check for ${email}: ${data.heard_about_us === null ? 'NULL' : data.heard_about_us}`);
        return data.heard_about_us === null;
    }
}
