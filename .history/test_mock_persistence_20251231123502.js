...
async function testMockFlow() {
    ...
    const userKey = `guest-test-${Date.now()}`; // Use guest- prefix to avoid UUID validation
    let sessionId = null;
    ...
    try {
        ...
        if (!ansData.success) {
            ...
            if (ansData.error) console.error("   Error Details:", JSON.stringify(ansData.error, null, 2));
        }
    } catch (e) {
