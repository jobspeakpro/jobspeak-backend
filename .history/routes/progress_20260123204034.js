...
router.get("/progress/summary", async (req, res) => {
    try {
        ...
        const { count: total_practice_sessions } = await supabase
            .from('activity_events')
            .select('*', { count: 'exact', head: true })
            .eq('activity_type', 'practice')
            .or(`user_id.eq.${usedValue},identity_key.eq.${usedValue}`); // Simplify since usedValue is the resolved one

        // 2. Days practiced (distinct dates from activity_events)
        const { data: practiceDates } = await supabase
            .from('activity_events')
            .select('day')
