...
router.get("/progress/summary", async (req, res) => {
    try {
        ...
        if (!sessions || sessions.length === 0) {
            // Check legacy sessions first, if empty, proceed to activity_events check below
            // Actually, we should merge them, but for now activity_events is the source of truth for new system
        }

        // 1. Total practice sessions (from activity_events)
        ...
        if (user_id) {
            ...
            practiceQuery = practiceQuery.eq('identity_key', identity_key);
        }

        const { count: practiceCount, error: practiceErr } = await practiceQuery;
        if (practiceErr) console.error("Practice count error:", practiceErr);
        const total_practice_sessions = practiceCount || 0;
        console.log(`[PROGRESS] Practice Count: ${total_practice_sessions} for usedValue=${usedValue}`);

        // 2. Days practiced (distinct dates from activity_events)
        let datesQuery = supabase
            .from('activity_events')
            .select('day');

        if (user_id) {
            datesQuery = datesQuery.eq('user_id', user_id);
        } else if (identity_key) {
            datesQuery = datesQuery.eq('identity_key', identity_key);
        }
        datesQuery = datesQuery.order('day', { ascending: false });

        const { data: practiceDates, error: datesErr } = await datesQuery;
        if (datesErr) console.error("Dates query error:", datesErr);

        const uniqueDates = new Set((practiceDates || []).map(d => d.day));
        ...
        if (user_id) {
            ...
            recentQuery = recentQuery.eq('identity_key', identity_key);
        }

        const { data: recentEvents, error: recentErr } = await recentQuery.order('created_at', { ascending: false }).limit(3);
        if (recentErr) console.error("Recent events error:", recentErr);
