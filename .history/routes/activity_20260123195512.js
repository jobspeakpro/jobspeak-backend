...
export function resolveIdentity(req) {
    ...
    // Priority 2: x-guest-key header (if not authenticated)
    if (!user_id) {
    ...
    // Fallback 1: body.userKey (NO heuristics, just plain value if header missing)
    if (!user_id && !identity_key && req.body && req.body.userKey) {
        identity_key = req.body.userKey;
        mode = 'guest';
    }

    // Fallback 2: query.userKey (NO heuristics, restore support for GET params)
    if (!user_id && !identity_key && req.query && req.query.userKey) {
        identity_key = req.query.userKey;
        mode = 'guest';
    }
    ...
    return {
        ...
        identity_key,
        mode,
        usedValue: user_id || identity_key
    };
}
...
/**
 * POST /api/activity/debug-seed
 * Force insert 1 practice and 1 mock interview event for the current identity.
 * GUARDED BY ENV VAR: ACTIVITY_DEBUG_SEED=1
 */
router.post("/activity/debug-seed", async (req, res) => {
    try {
        if (process.env.ACTIVITY_DEBUG_SEED !== '1') {
        ...
        const events = [
            {
                ...
                context: { debug: true, seed: Date.now(), tabId: `seed-${Date.now()}-1` },
            ...
            },
            {
                ...
                context: { debug: true, seed: Date.now(), tabId: `seed-${Date.now()}-2` },
                day: new Date().toISOString().split('T')[0]
            }
        ];
        ...
        if (error) {
            console.error('[ACTIVITY-SEED] Error:', error);
        ...
        const { count: totalToday } = await supabase
            ...
            .or(`user_id.eq.${user_id || 'null'},identity_key.eq.${identity_key || 'null'}`)
        ...
        console.error('[ACTIVITY-SEED] Unexpected error:', error);
        return res.status(500).json({ error: 'Internal seed error' });
    }
});
