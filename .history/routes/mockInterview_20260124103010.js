...
import { processReferralAction } from "./referrals.js";
...
router.get("/mock-interview/limit-status", async (req, res) => {
    try {
        // SECURITY: Get authenticated user from Authorization header (JWT token)
        // Does NOT accept client-provided userKey - prevents spoofing
        const { userId, isGuest } = await getAuthenticatedUser(req);

        // CRITICAL: Require authentication for mock interviews
        // Guests/unauthenticated users must sign up to prevent incognito loophole
        if (isGuest || !userId) {
            console.log(`[MOCK LIMIT STATUS] Unauthenticated request - requiring auth`);
            return res.json({
                canStartMock: false,
                isGuest: true,
        ...
        console.log(`[MOCK LIMIT STATUS] Checking eligibility for authenticated user: ${userId}`);
...
router.post("/mock-interview/answer", upload.single('audioFile'), async (req, res) => {
    try {
        ...
        // Determine user identity securely
...
router.get("/mock-interview/summary", async (req, res) => {
    try {
        ...
        if (updateError) {
            ...
            // Check if this qualifies for a referral credit grant
            if (sessionData && sessionData.user_id) {
                // Async fire-and-forget (safe, handled in helper)
                processReferralAction(sessionData.user_id).catch(err =>
                    console.error('[MOCK SUMMARY] Referral processing error:', err)
                );
            }
        }
