 ...
 */
function validateAndFixStrongerExample(strongerExample, usedVocabSet, questionText) {
    ...
    // If text is empty or contaminated, regenerate it
    if (!text || text.length < 50 || text.includes('[redacted]')) {
...
function generatePerQuestionBreakdown(attempts) {
    ...
    return (attempts || []).map((attempt, index) => {
        ...
        // 3. Generate initial stronger example structure
        let strongerExample = {
        ...
        // 4. CRITICAL: Validate and fix stronger example
