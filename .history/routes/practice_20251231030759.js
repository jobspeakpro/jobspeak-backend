...
router.post("/practice/answer", async (req, res) => {
    try {
        ...
        const vocabulary = generateRoleVocabulary(questionText, answerText || '');
        const clearerRewrite = generateSTARRewrite(questionText, answerText || '', evaluation.score, evaluation.feedback);

        // Generate TTS audio for clearer rewrite
        let clearerRewriteAudioUrl = null;
        try {
            if (clearerRewrite.text) {
                const audioBuffer = await generateTTS(clearerRewrite.text);
