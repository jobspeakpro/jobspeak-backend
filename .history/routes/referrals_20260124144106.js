...
export async function processReferralAction(userId) {
    ...
    try {
        ...
        const referrerId = referralLog.referrer_user_id;
...
router.get('/referrals/code', async (req, res) => {
    try {
        ...
        if (!profile || !profile.referral_code) {
            ...
            if (updateError) {
                // Determine if we need to insert instead (if profile is missing, though trigger handles it)
                // If update failed because row likely exists (from trigger), it might be a collision? 
                // Retrying once with new code or handling insert if no profile (rare)
                return res.status(500).json({ error: 'Failed to generate code', details: updateError.message });
            }
            return res.json({ referralCode: newCode });
        }

        return res.json({ referralCode: profile.referral_code });
    } catch (err) {
        console.error(err);
