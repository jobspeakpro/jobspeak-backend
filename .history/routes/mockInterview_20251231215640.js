...
function varyBullets(baseBullets, index, type) {
    // Current simple implementation: return base bullets
    // This hook allows future template rotation based on index
...
function generatePerQuestionBreakdown(attempts) {
    ...
    return (attempts || []).map((attempt, index) => {
        ...
        const cleanText = stripHtml(attempt.clearer_rewrite || "");
    ...
    try {
        ...
        const perQuestion = (attempts || []).map((attempt, index) => {
            ...
            return {
                ...
                transcript: attempt.answer_text || "",
                score: attempt.score || 0,
                ...
                strongerExample: {
                    text: (attempt.clearer_rewrite || "").replace(/<\/?[^>]+(>|$)/g, ""),
                    vocab: vocab.map(v => ({
                        word: v.word,
                        ipa: v.ipa,
                        accent: "US",
