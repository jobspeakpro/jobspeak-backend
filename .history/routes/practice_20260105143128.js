...
router.post(["/practice/answer", "/answer"], async (req, res) => {
    ...
    try {
        ...
        if (resolvedKey && !isPro) {
            // Generate idempotency key: hash of resolvedKey + questionId + normalized answer text
        ...
        // Add debug field to response
        response.debug = {
            identityType: identityType,
            identityKey: resolvedKey,
            identitySource: identitySource,
            used: response.usage ? response.usage.used : 0,
            limit: response.usage ? response.usage.limit : (isPro ? -1 : 3)
        };
