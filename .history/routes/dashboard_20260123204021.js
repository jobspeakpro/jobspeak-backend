...
router.get("/dashboard/summary", async (req, res) => {
    try {
        ...
        // Get total practice sessions (from activity_events)
        const practiceCountQuery = supabase
            .from('activity_events')
            .select('*', { count: 'exact', head: true })
            .eq('activity_type', 'practice');

        let practiceQuery = practiceCountQuery;
        if (userId && guestKey) {
            practiceQuery = practiceQuery.or(`user_id.eq.${userId},identity_key.eq.${guestKey}`);
        } else if (userId) {
            practiceQuery = practiceQuery.eq('user_id', userId);
        } else if (guestKey) {
            practiceQuery = practiceQuery.eq('identity_key', guestKey);
        }
        const { count: practiceCount } = await practiceQuery;
        const total_practice_sessions = practiceCount || 0;

        // Get total mock interviews (from activity_events)
        const mockCountQuery = supabase
            .from('activity_events')
            .select('*', { count: 'exact', head: true })
            .eq('activity_type', 'mock_interview');

        let mockQuery = mockCountQuery;
        if (userId && guestKey) {
            mockQuery = mockQuery.or(`user_id.eq.${userId},identity_key.eq.${guestKey}`);
        } else if (userId) {
            mockQuery = mockQuery.eq('user_id', userId);
        } else if (guestKey) {
            mockQuery = mockQuery.eq('identity_key', guestKey);
        }
        const { count: mockCount } = await mockQuery;
        const total_mock_interviews = mockCount || 0;

        // Get last mock interview
        const lastMock = getLastMockInterview(usedValue); // Keep legacy for detailed object if available, otherwise could query events
