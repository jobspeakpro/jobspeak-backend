...
router.post('/affiliate/apply', async (req, res) => { // Changed path back to original '/affiliate/apply'
    try {
        ...
        // Update DB with notification status (Best effort - fire & forget update)
        if (emailResult) {
            const updatePayload = {};
            if (emailResult.skipped) {
                updatePayload.notification_status = 'skipped';
                updatePayload.notification_error = emailResult.reason;
            } else if (emailResult.success) {
                updatePayload.notification_status = 'sent';
            } else if (emailResult.error) {
                updatePayload.notification_status = 'failed';
                updatePayload.notification_error = emailResult.message;
            }

            // Perform update if we have a status change
            if (updatePayload.notification_status) {
                await supabase
                    .from('affiliate_applications')
