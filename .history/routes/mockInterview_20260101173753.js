...
router.get("/mock-interview/limit-status", async (req, res) => {
    try {
        // Resolve userKey from header (preferred), body, or query
        // Priority: header (x-user-key) > body > query
        const userKey =
            req.header('x-user-key') ||
            req.body?.userKey ||
            req.query?.userKey;

        // If no userKey provided, treat as guest
        if (!userKey) {
            console.log(`[MOCK LIMIT STATUS] No userKey provided - treating as guest`);
            return res.json({
                canStartMock: true,
                isGuest: true
            });
