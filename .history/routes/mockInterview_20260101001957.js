...
function pickUniqueVocabWords(candidateWords, usedSet) {
    ...
    // Expanded list to ensure enough unique words for 10 questions (needs ~20 words if attempts have none)
            ...
            if (!usedSet.has(lower)) {
                ...
                if (VOCAB_DATA[lower]) {
                    ...
                    uniqueVocab.push({
                        ...
                        audioText: fb,
                        pos: "verb",
                        definition: "To take effective action", // Generic filler
                        example: `I utilized my skills to ${fb.toLowerCase()} the process.`
                    });
                }
                usedSet.add(lower);
            }
    ...
    // Extreme fallback: If we somehow still don't have 2 (e.g. session > 10 q's + empty attempts)
    // We allow repeats but prioritize uniqueness. Or just fail gracefully with generic placeholders
    // that won't break uniqueness constraints technically if we don't add them to set?
    // But user wants "No vocab repeats ... session". 
    // If we are truly out of words, we must return something.
    // Let's assume the 20+ list is sufficient for max 12 questions.
...
function generatePerQuestionBreakdown(attempts) {
    ...
    return (attempts || []).map((attempt, index) => {
        ...
        // 5. Ensure transcript - check all possible persistence fields
...
router.post("/mock-interview/answer", upload.single('audioFile'), async (req, res) => {
    try {
        ...
        // Normalize answer text from multiple possible field names
        // Priority: answer_text > answerText > answer > transcript
        const normalizedAnswer = req.body.answer_text || req.body.answerText || req.body.answer || req.body.transcript || answerText || '';

        console.log(`[MOCK ANSWER] Normalized answer length: ${normalizedAnswer?.length || 0}`);
...
router.get("/mock-interview/summary", async (req, res) => {
    try {
        ...
        // CRITICAL: attemptCount must equal actual unique attempts (not inflated by duplicates)
        const actualAttemptCount = attempts?.length || 0;
