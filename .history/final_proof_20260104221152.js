...
const API_BASE = 'https://jobspeak-backend-production.up.railway.app'; // Or jobspeakpro.com, using direct railway for reliability or actual domain
const PROD_URL = 'https://jobspeakpro.com';

// Use Service Role for Admin/Reset tasks
const supabaseAdmin = createClient(SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY);
// Use Anon Key for Auth login
...
async function runProof() {
    console.log('🔍 STARTING FINAL PROOF...\n');
    ...
    // 2. Reset jsp.qa.001 to NULL first (to ensure clean slate for test)
    ...
    console.log(`   DATA: ${JSON.stringify(firstPayload)}`);

    const res1 = await fetch(`${PROD_URL}/api/profile/heard-about`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}` // In verify script it used Bearer
        },
    ...
    console.log(`   CURL POST ${PROD_URL}/api/profile/heard-about`);
    console.log(`   DATA: ${JSON.stringify(secondPayload)}`);
    ...
    // Verify
    await verifyNull(QA_USER_EMAIL);
    await verifyNull(QA_USER_2_EMAIL);
...
async function setHeardAboutToNull(email) {
    ...
    else console.log(`   ✅ Reset ${email} to NULL`);
}
...
async function verifyNull(email) {
    ...
    if (error) console.log(`   ❌ Verify Error: ${error.message}`);
        ...
        console.log(`   🔍 DB Check for ${email}: heard_about_us = ${data.heard_about_us}`);
        if (data.heard_about_us === null) console.log(`   ✅ CONFIRMED NULL`);
        else console.log(`   ❌ FAILED: Value is ${data.heard_about_us}`);
    }
}
