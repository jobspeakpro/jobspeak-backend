...
export function generateSTARRewrite(questionText, answerText, score, feedback, forcedVocab = null) {
    ...
    if (contextType === 'conflict') {
        rewrite = `In a past situation, I disagreed with a colleague on the best approach. ${userContext}, but we hit a wall. I knew we needed alignment, so I asked questions to understand their perspective. I decided to ${buildActionPhrase(v1)} to bridge the gap. We found a compromise that worked for everyone. This choice ${buildResultPhrase(v2)} and kept the relationship professional.`;
    }
    else if (contextType === 'failure') {
        rewrite = `I once made a mistake where I missed a key detail. ${userContext}. I took immediate ownership rather than making excuses. I communicated the issue and decided to ${buildActionPhrase(v1)} to fix it. I also documented the error to prevent recurrence. This experience taught me to ${buildResultPhrase(v2)}, making me a more reliable professional.`;
    }
    else if (contextType === 'leadership') {
        rewrite = `I noticed the team was facing a blocked workflow. ${userContext}. I stepped in to provide clear direction while giving them space to own the work. I took specific steps to ${buildActionPhrase(v1)} so everyone felt supported. By checking in regularly, I ${buildResultPhrase(v2)}. The team delivered the project successfully and morale remained high.`;
    }
    else if (contextType === 'weakness') {
        rewrite = `I used to struggle with taking on too much. ${userContext}. I realized I needed a better system, so I started prioritizing my day differently. I decided to ${buildActionPhrase(v1)} to stay organized. This shift ${buildResultPhrase(v2)} with my stakeholders. Call it a lesson learned in sustainable working.`;
    }
    else {
        // Default / Challenge
        rewrite = `In a previous role, ${userContext}. I assessed the situation and noticed a clear friction point. I knew I needed to act, so I ${buildActionPhrase(v1)} to hit the ground running. I broken the problem down and ${buildResultPhrase(v2)}. By staying focused on the goal, I drove a strong, measurable result.`;
    }
