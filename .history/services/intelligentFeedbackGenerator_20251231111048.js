...
export function generateSTARRewrite(questionText, answerText, score, feedback) {
    ...
    // 1. If profanity or very short, use Role-Specific Perfect Templates with POS awareness
    if (hasProfanity || wordCount < 20) {
        // Helper to insert based on POS
        const insert = (v) => { // validation helper
            return `<u>${v.word.toLowerCase()}</u>`;
        };

        if (role === 'engineer') {
            return {
                text: `In my last project, I had to ${vocab1.pos.includes('verb') ? `<u>${word1}</u>` : `improve <u>${word1}</u> for`} a complex system. I noticed performance was dropping, so I paused to analyze the root cause. I decided to ${vocab2.pos.includes('verb') ? `<u>${word2}</u>` : `prioritize <u>${word2}</u> in`} the database queries to reduce load. This allowed us to handle 3x more users without crashing. The fix improved overall system stability by 40%.`,
                hireProbabilityEstimate: 85
            };
        } else if (role === 'dentist') {
            return {
                text: `I had a patient who was very anxious. To help them relax, I used a soft tone to ${vocab1.pos.includes('verb') ? `<u>${word1}</u>` : `build <u>${word1}</u> with`} them before we started. I focused on ways to ${vocab2.pos.includes('verb') ? `<u>${word2}</u>` : `enhance <u>${word2}</u> during`} the treatment. This approach helped them feel safe. As a result, the procedure went smoothly and they left with a smile.`,
                hireProbabilityEstimate: 85
            };
        } else {
            return {
                text: `In a past role, I had to ${vocab1.pos.includes('verb') ? `<u>${word1}</u>` : `address <u>${word1}</u>`} on a tight deadline. I realized we were at risk, so I gathered the team. We decided to ${vocab2.pos.includes('verb') ? `<u>${word2}</u>` : `improve <u>${word2}</u>`} to remove unnecessary steps. This helped us move faster without making mistakes. In the end, we delivered on time and improved efficiency by 20%.`,
                hireProbabilityEstimate: 80
            };
        }
    }

    // 2. For decent length answers, construct natural rewrite with POS-aware injection
    ...
    // Add middle section with vocab 1 (POS aware)
    if (vocab1.pos.includes('verb')) {
        rewrite += `I knew I needed to take action, so I focused on ways to <u>${word1}</u> the situation. `;
    } else {
        rewrite += `I knew I needed to take action, so I focused on improving <u>${word1}</u>. `;
    }

    // Add action/result section with vocab 2 (POS aware)
    if (vocab2.pos.includes('verb')) {
        rewrite += `I broke the problem down and made sure to <u>${word2}</u> effectively. `;
    } else {
        rewrite += `I broke the problem down and made sure to prioritize <u>${word2}</u>. `;
    }

    rewrite += `By staying focused, I was able to drive a positive outcome. `;
