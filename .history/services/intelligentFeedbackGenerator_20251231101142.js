...
export function generateSTARRewrite(questionText, answerText, score, feedback) {
    ...
    if (hasProfanity || wordCount < 15) {
        ...
        enhanced = `In a professional context, I would approach this by systematically addressing the challenge. I took responsibility for analyzing the situation and implementing a targeted solution. Through strategic action and stakeholder alignment, I achieved measurable results that improved key outcomes by approximately 30%.`;
...
/**
 * Generate blunt, helpful hiring manager interpretation (softened tone)
 */
export function generateHiringManagerInterpretation(score, feedback, answerText) {
    ...
    // Determine seniority perception (avoid harsh "junior" language)
    let senioritySignal = 'signals early-career level';
    if (score >= 75 && hasMetrics && feedback.structure >= 18) senioritySignal = 'reads as senior-level';
    else if (score >= 60 && (hasMetrics || feedback.structure >= 15)) senioritySignal = 'suggests mid-level experience';

    // Build blunt, helpful interpretation with professional tone
    if (score >= 80) {
        return `The candidate clearly knows what they're doing. ${hasMetrics ? 'Quantified results show impact awareness.' : 'Good structure but would like to see more metrics.'} ${senioritySignal === 'reads as senior-level' ? 'Reads as senior-level.' : 'Solid mid-level.'} Would interview. Low risk.`;
    } else if (score >= 60) {
        return `Decent answer but ${!hasMetrics ? 'missing metrics makes it hard to gauge actual impact' : 'structure needs strengthening to follow the story clearly'}. ${senioritySignal === 'suggests mid-level experience' ? 'Probably mid-level but needs validation.' : 'Needs stronger impact signals.'} Moderate risk - need more data points.`;
    } else if (score >= 40) {
        return `Candidate can communicate but lacks specifics. ${!hasMetrics ? 'No metrics provided - can\'t tell if they actually moved the needle.' : ''} ${!hasSTAR ? 'Missing clear story structure.' : ''} ${senioritySignal}. Would pass unless desperate. Higher risk.`;
    } else {
