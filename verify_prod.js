...
async function verifyProd() {
    ...
    // 0. Migration (Manual Trigger)
    logRoute("=== Step 0: Trigger Manual Migration ===");
    try {
        const migRes = await fetch(`${BACKEND_URL}/__admin/migrate`, {
            method: 'POST',
            headers: { 'x-admin-secret': 'temporary_migration_key_2026' }
        });
        const migText = await migRes.text();
        logRoute(`Migration Response: ${migRes.status} ${migText}`);
        if (migRes.status === 200) log("✅ Migration Triggered Successfully.");
        else log("⚠️ Migration Trigger returned " + migRes.status);
    } catch (e) { logRoute(`Migration Error: ${e.message}`); }

    // 1. Route Checks (curl equivalent)
    log("Running Route Checks...");

    // Check 1: GET /api/referrals/me (Expect 401 Unauthorized, NOT 404)
    try {
        logRoute(`=== CHECK 1: GET ${BACKEND_URL}/api/referrals/me ===`);
        const resME = await fetch(`${BACKEND_URL}/api/referrals/me`);
        logRoute(`HTTP/${resME.status} ${resME.statusText}`);
        const text = await resME.text();
        logRoute(`Response Body: ${text.substring(0, 500)}`); // Log first 500 chars

        // 401 means Route Exists but needs auth. 404 means route missing.
        if (resME.status !== 404) log("✅ Route /api/referrals/me EXISTS.");
        else log("❌ Route /api/referrals/me NOT FOUND.");
    } catch (e) { logRoute(`Error: ${e.message}\n`); }

    // Check 2: POST /api/affiliate/apply (Validation Check, expects 400 or 200)
    try {
        logRoute(`=== CHECK 2: POST ${BACKEND_URL}/api/affiliate/apply ===`);
        const res1 = await fetch(`${BACKEND_URL}/api/affiliate/apply`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({}) // Empty body -> 400
        });
        logRoute(`HTTP/${res1.status} ${res1.statusText}`);
        const t1 = await res1.text();
        logRoute(`Body: ${t1.substring(0, 500)}`);

        if (res1.status !== 404) log("✅ Route /api/affiliate/apply EXISTS.");
        else log("❌ Route /api/affiliate/apply NOT FOUND.");
    } catch (e) { logRoute(`Error: ${e.message}\n`); }

    // ... rest of auth and E2E ...
