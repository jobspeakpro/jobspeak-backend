...
function New-TestUser {
    ...
    try {
        ...
        return @{ Email = $email; Token = $loginRes.access_token }
    }
    catch {
...
$referrer = New-TestUser "referrer_v2"
...
$referee = New-TestUser "referee_v2"

try {
    Write-Host "CLAIM REQUEST ($($referee.Email) -> $refCode)"
...
try {
    Write-Host "HISTORY REQUEST ($($referrer.Email))"
