...
async function runVerification() {
    ...
    // Try fetching existing users first (faster, reliable)
    const { data: { users }, error: listError } = await supabase.auth.admin.listUsers();

    if (!listError && users && users.length >= 2) {
        userA_ID = users[0].id;
        userB_ID = users[1].id;
        console.log(`✅ Using existing users: ${users[0].email} -> ${users[1].email}`);
    } else {
        // Fallback to creation
        const emailA = `verify_ref_${Date.now()}@example.com`;
        const emailB = `verify_usr_${Date.now()}@example.com`;
    ...
    if (!userA_ID || !userB_ID) {
        console.error("❌ FAILURE: Could not obtain 2 test users.");
        process.exit(1);
    }
    console.log(`✅ Test Users Ready: Referrer (${userA_ID}) -> Referee (${userB_ID})`);
